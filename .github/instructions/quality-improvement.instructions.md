...
| Q05.9 | in-progress | Improve state-machine semantic accuracy: initial/final states, composite states, transitions, events, guards, effects, entry/do/exit actions, and invalid transition diagnostics. | `TestUMLStateMachineFinalRendersFinalDot`; `uml-state-machine.xal` validates and renders |
| Q05.10 | in-progress | Improve state-machine design quality: state shape proportions, nested-state padding, transition bend points, guard/effect label placement, and final-node readability. | `TestUMLStateMachinePseudostatesKeepCompactProportions`; `TestUMLStateMachineRowsSeparateBranches`; state-machine final nodes render as compact UML final states with an inner dot; `uml-state-machine.xal` models the EC order status flow with row-separated normal/refund/return/cancel branches and regenerated `docs/src/images/uml-state-machine.svg` |
