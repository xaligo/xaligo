...
| Q05.13 | in-progress | Improve interaction, communication, timing, object, use-case, and profile diagrams for supported UML notation and explicit unsupported-profile diagnostics. | `TestUMLInteractionOverviewRendersInteractionReferences`; interaction-overview diagrams follow activity-style control flow with initial/final/decision/merge/fork/join nodes, `interaction notation="ref|sd"` headers, separated note bodies, and standard final inner dots; regenerated `docs/src/images/uml-interaction-overview.svg`; communication/timing/object/use-case/profile passes remain pending |
