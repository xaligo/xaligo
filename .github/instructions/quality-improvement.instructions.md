...
| Q05.8 | in-progress | Improve sequence-diagram design quality: timeline spacing, activation contrast, message arrow style, return-line distinction, lifeline header readability, and vertical density. | Participants and lifelines render as compact orange headers with dashed vertical axes; sequence messages render destination activation bars with deep-blue outlines and white bodies; synchronous messages use filled triangle arrows; asynchronous messages use open arrows in SVG/plan output; return messages are dashed response connectors; destroy messages add destination stop markers; sequence metadata is retained in editable scene output |
