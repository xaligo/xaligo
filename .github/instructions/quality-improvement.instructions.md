...
| Q05.6 | in-progress | Improve class-diagram design quality: compartment rhythm, long member wrapping, stereotype readability, relation label spacing, crow-foot/diamond marker clarity, and dense-layout crossing reduction. | Class classifiers use compact multi-row placement, real header divider lines, and the xaligo activity palette; relation lines use deep blue defaults; `TestUMLClassStereotypeAndModifiersReachEditableScene` asserts compact shape/text/divider styling; `uml-class.xal` includes a primary aggregate-root classifier |
...
| Q05.16 | not-started | Refresh UML documentation and examples after implementation slices, including generated images and command accuracy. | `mdbook build docs`; regenerated UML docs assets where tracked |
