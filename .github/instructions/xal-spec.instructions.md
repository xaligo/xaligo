...
|---|---|---|---|
| `class-diagram` | `class`, `interface`, `enumeration` | `association`, `aggregation`, `composition`, `generalization`, `realization`, `dependency` | Requires one classifier. Classifiers accept `stereotype`, `abstract="true|false"`, and `static="true|false"`; stereotypes and enabled modifiers are visible classifier-label metadata. Aggregation/composition are class to class; generalization joins equal classifier kinds; realization is class to interface. |
