...
|---|---|---|---|
| `class-diagram` | `class`, `interface`, `enumeration` | `association`, `aggregation`, `composition`, `generalization`, `realization`, `dependency` | Requires one classifier. Classifiers accept `stereotype`, `abstract="true|false"`, `static="true|false"`, and `tone="primary"`; stereotypes and enabled modifiers are visible classifier-label metadata. Classifiers use compact multi-row placement, real header divider lines, and the xaligo activity palette by default. Relation lines use the same deep-blue default. Aggregation/composition are class to class; generalization joins equal classifier kinds; realization is class to interface. |
