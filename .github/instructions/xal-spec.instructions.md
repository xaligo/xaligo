...
| `component-diagram` | `component`, `interface`, `port`, `artifact` | `dependency`, `realization`, `association`, `assembly`, `delegation` | Requires one component. Component boxes render the UML component notation with two small rectangles on the left using the existing xaligo deep-blue/white palette, and component child `provided-interface` / `required-interface` entries render as boundary lollipop/socket symbols with outside labels. Required sockets are direct semicircle lines, not masked circles. Component diagrams use compact grid placement by default. Realization is component to interface. Assembly uses port/interface endpoints and includes a port. A port requires a component owner and renders on that owner boundary. Delegation is port to component/port. |
