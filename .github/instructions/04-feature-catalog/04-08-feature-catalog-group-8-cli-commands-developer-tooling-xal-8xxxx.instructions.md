...
| XAL-8000150 | `xaligo lsp` command | Implemented | Runs an LSP 3.18 server over stdio with full-document sync, push/pull diagnostics, hierarchical document symbols, project-backed workspace symbols, semantic tokens, and hover, all through the shared project analysis boundary. |
| XAL-8000160 | `xaligo rag` command family | Implemented | Incrementally indexes only Markdown below `docs/` into the local SQLite/FTS5 project database and provides `index`, `search`, and polling `watch`; explicit `.xal` analysis is separate from initial corpus registration. |
| XAL-8000170 | `xaligo mcp` command | Implemented | Exposes shared validation, explicit concept inspection, SVG rendering, docs-only indexing, project search, and icon registry operations through stateless MCP 2026-07-28 stdio and localhost-only Streamable HTTP transports. |
