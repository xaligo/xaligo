...
func TestBuildPlanPreservesConnectorStylesAndLegend(t *testing.T) {
	opacity := 100.0
	scene := usecase.Scene{Elements: []usecase.Element{
		...
		{ID: "route", Type: "arrow", StrokeColor: "#64748b", StrokeWidth: 1, Opacity: &opacity,
			StartBinding: &usecase.Binding{ElementID: "src-item", FixedPoint: []float64{1, 0.5}},
			EndBinding:   &usecase.Binding{ElementID: "dst-item", FixedPoint: []float64{0, 0.5}},
			CustomData:   &usecase.CustomData{ConnectorKind: "route"}},
		{ID: "traffic", Type: "arrow", StrokeColor: "#2563eb", StrokeWidth: 2, StrokeStyle: "dotted", Opacity: &opacity,
			StartBinding: &usecase.Binding{ElementID: "src-item", FixedPoint: []float64{1, 0.5}},
			EndBinding:   &usecase.Binding{ElementID: "dst-item", FixedPoint: []float64{0, 0.5}},
			CustomData:   &usecase.CustomData{ConnectorKind: "traffic", ConnectorStartArrowhead: "oval", ConnectorEndArrowhead: "diamond"}},
	}, AppState: &usecase.AppState{ViewBackgroundColor: "#FFFFFF"}}

	plan := usecase.BuildPlan(&scene, usecase.Options{PxPerInch: 96, ArrowStyle: "thin"})
...
func TestBuildPlanPaperMarginsInsetFittedContent(t *testing.T) {
	...
	plan := usecase.BuildPlan(&scene, usecase.Options{
...
func TestBuildPlanDrawsHeaderTagAboveConnectors(t *testing.T) {
	...
	scene := usecase.Scene{Elements: []usecase.Element{
		...
		{ID: "group-rect", Type: "rectangle", X: 0, Y: 0, Width: 160, Height: 80, StrokeColor: "#00A1C9", BackgroundColor: "transparent", Opacity: &opacity, CustomData: &usecase.CustomData{GroupBorder: true}},
		{ID: "raw-line", Type: "line", X: 0, Y: 20, Width: 160, Height: 0, StrokeColor: "#334155", Opacity: &opacity, Points: [][]float64{{0, 0}, {160, 0}}},
		{ID: "header-bg", Type: "line", X: 0, Y: 8, Width: 80, Height: 24, StrokeColor: "#00A1C9", BackgroundColor: "#FFFFFF", Opacity: &opacity, Points: [][]float64{{0, 0}, {68, 0}, {80, 12}, {68, 24}, {0, 24}, {0, 0}}, CustomData: &usecase.CustomData{GroupHeader: true}},
	}}
	plan := usecase.BuildPlan(&scene, usecase.Options{PxPerInch: 96})
...
func TestBuildPlanJSONEncodesPlan(t *testing.T) {
	...
	out, err := usecase.BuildPlanJSON(sceneJSON, usecase.Options{PxPerInch: 100})
	if err != nil {
		t.Fatal(err)
	}
