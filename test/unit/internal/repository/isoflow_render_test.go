...
func TestRenderBuildsIsometricDocument(t *testing.T) {
  ...
  "appState":{"viewBackgroundColor":"#ffffff"}
	...
	out, err := repository.NewIsoflowRepository().Render(scene)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderWithGeneratedIconsPreservesServiceNameLabels(t *testing.T) {
  ...
  "files":{"lambda":{"dataURL":"data:image/svg+xml;base64,QQ=="}}
	...
	out, err := repository.NewIsoflowRepository().RenderWithIcons(scene, map[string]string{"lambda": "data:image/svg+xml;base64,PHN2Zz5fX1hBTElHT19MQUJFTF9fPC9zdmc+"})
	if err != nil {
		t.Fatal(err)
	}
	var document entity.IsoflowDocument
