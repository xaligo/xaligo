		...
		if gd, isGroup := awsGroupsV1EngineSceneTypes[b.Tag]; isGroup {
			...
			*elements = append(*elements, map[string]any{
			...
			if sequenceLifeline {
				...
				if classShape {
					boundElements = umlClassBoundElementsV1EngineSceneWalk(b)
				}
...
func appendUMLClassCompartmentsV1EngineSceneWalk(box *entity.Box, elements *[]map[string]any, updated int64) {
	...
	headerText, sections := umlClassTextSectionsV1EngineSceneWalk(box)
	headerH := umlClassHeaderHeightV1EngineSceneWalk(box, lineHeight, headerText, len(sections) > 0)
	...
	*elements = append(*elements, map[string]any{
	...
	for index, section := range sections {
		sectionH := math.Max(lineHeight, float64(section.Lines)*lineHeight+6)
		textH := math.Max(1, sectionH-3)
		if index == len(sections)-1 {
			textH = math.Max(1, bottomY-sectionY)
		}
		customData := map[string]any{"xaligoUmlClassOperationContent": true}
		if section.Kind == "attribute" {
			customData = map[string]any{"xaligoUmlClassAttributeContent": true}
		}
		appendUMLClassTextV1EngineSceneWalk(
			elements,
			fmt.Sprintf("%s-class-%s-text-%d", box.ID, section.Kind, index),
			textX,
			sectionY,
			textW,
			textH,
		...
		dividerY := sectionY + sectionH
		appendUMLClassBodyDividerV1EngineSceneWalk(box, elements, updated, index, dividerY)
		sectionY = dividerY + 6
	}
}
...
func appendUMLClassBodyDividerV1EngineSceneWalk(box *entity.Box, elements *[]map[string]any, updated int64, index int, y float64) {
	id := fmt.Sprintf("%s-class-body-divider-%d", box.ID, index)
	seed := stableSceneSeedV1EngineSceneTypes(id)
	*elements = append(*elements, map[string]any{
		"id": id, "type": "line",
		"x": box.X, "y": y, "width": box.W, "height": 0,
...
func umlClassTextSectionsV1EngineSceneWalk(box *entity.Box) (string, []umlClassSectionV1EngineParseUml) {
	lines := strings.Split(box.Label, "\n")
	sections := umlClassSectionsFromAttrsV1EngineParseUml(box.Attrs)
	...
	if headerLines < 1 || headerLines > len(lines) {
		...
		headerLines = len(lines) - bodyLines
		if headerLines < 1 || headerLines > len(lines) {
			headerLines = 1
		}
	}
...
func umlClassBoundElementsV1EngineSceneWalk(box *entity.Box) []map[string]any {
	headerText, sections := umlClassTextSectionsV1EngineSceneWalk(box)
	boundElements := make([]map[string]any, 0, 1+len(sections))
	if strings.TrimSpace(headerText) != "" {
		boundElements = append(boundElements, map[string]any{
			"type": "text",
			"id":   fmt.Sprintf("%s-class-header-text", box.ID),
		})
	}
	for index, section := range sections {
		...
		boundElements = append(boundElements, map[string]any{
			"type": "text",
			"id":   fmt.Sprintf("%s-class-%s-text-%d", box.ID, section.Kind, index),
		})
	}
	return boundElements
}

func umlClassHeaderHeightV1EngineSceneWalk(box *entity.Box, lineHeight float64, headerText string, hasSections bool) float64 {
	lines := strings.Count(headerText, "\n") + 1
	maximumHeight := box.H
	if hasSections {
		maximumHeight = box.H * 0.48
	}
	return math.Min(math.Max(34, 10+float64(lines)*lineHeight), math.Max(34, maximumHeight))
}
...
func appendUMLComponentBoundaryInterfacesV1EngineSceneWalk(box *entity.Box, elements *[]map[string]any, updated int64) {
	interfaces := splitUMLComponentInterfacesV1EngineSceneWalk(box.Attrs["uml-component-interfaces"])
	appendUMLComponentInterfacesV1EngineSceneWalk(box, elements, updated, interfaces)
}

type umlComponentInterfaceV1EngineSceneWalk struct {
	label       string
	description string
}

func splitUMLComponentInterfacesV1EngineSceneWalk(value string) []umlComponentInterfaceV1EngineSceneWalk {
	var interfaces []umlComponentInterfaceV1EngineSceneWalk
	for _, line := range strings.Split(value, "\n") {
		...
		interfaces = append(interfaces, umlComponentInterfaceV1EngineSceneWalk{label: label, description: description})
	}
	return interfaces
}

func appendUMLComponentInterfacesV1EngineSceneWalk(box *entity.Box, elements *[]map[string]any, updated int64, interfaces []umlComponentInterfaceV1EngineSceneWalk) {
	if len(interfaces) == 0 {
		return
	}
...
func appendUMLComponentInterfaceDescriptionV1EngineSceneWalk(elements *[]map[string]any, id string, x, y, width, height float64, label, description string, box *entity.Box, updated int64, customData map[string]any) {
	seed := stableSceneSeedV1EngineSceneTypes(id)
	textID := id + "-text"
	*elements = append(*elements, map[string]any{
	...
	textCustomData := umlComponentInterfaceCustomDataV1EngineSceneWalk(box, label, map[string]any{"xaligoUmlComponentInterfaceDescriptionText": true})
	textCustomData["xaligoTextLayout"] = sceneTextLayoutV1EngineSceneBuild(entity.TextRoleLabel, true, 1.0)
	fontSize := math.Min(9, math.Max(7, attrFloatV1EngineLayoutAttributes(box.Attrs["font-size"], 14)-5))
	*elements = append(*elements, map[string]any{
		"id": textID, "type": "text",
		"x": x + 4, "y": y + 2, "width": math.Max(1, width-8), "height": math.Max(1, height-4),
		"angle": 0, "strokeColor": "#052d6e", "backgroundColor": "transparent",
		"fillStyle": "solid", "strokeWidth": 1, "strokeStyle": "solid",
		"roughness": 0, "opacity": 100,
		"groupIds": []string{}, "roundness": nil,
		"seed": stableSceneSeedV1EngineSceneTypes(textID), "version": 1, "versionNonce": stableSceneSeedV1EngineSceneTypes(textID),
		"isDeleted": false, "boundElements": nil,
		"updated": updated, "link": nil, "locked": false,
		"text": description, "fontSize": fontSize, "fontFamily": fontFamilyV1EngineSceneWalk(box.Attrs["font-family"]),
		"textAlign": "left", "verticalAlign": "middle",
		"containerId": id, "originalText": description, "lineHeight": 1.0,
		"customData": textCustomData,
	})
}
