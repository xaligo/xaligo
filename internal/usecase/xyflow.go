...
func RenderXYFlowScene(sceneJSON []byte) ([]byte, error) {
	...
	for _, element := range scene.Elements {
		...
		edge := XYFlowEdge{
			...
			Style:       map[string]any{"stroke": color, "strokeWidth": PositiveWidth(element.StrokeWidth), "strokeDasharray": cssStrokeDash(element.StrokeStyle)},
