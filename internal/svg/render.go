...
const defaultPxPerInch = 96.0

// RenderPlan converts the shared draw plan into an SVG document.
func RenderPlan(plan pptxplan.Plan, pxPerInch float64) ([]byte, error) {
	if pxPerInch <= 0 {
		pxPerInch = defaultPxPerInch
	}
...
func writeOp(b *bytes.Buffer, op pptxplan.DrawOp, ppi float64) {
...
func writeLine(b *bytes.Buffer, op pptxplan.DrawOp, x, y, w, h, ppi float64) {
	points := absolutePoints(op, x, y, w, h, ppi)
	if len(points) < 2 {
		return
	}
...
func absolutePoints(op pptxplan.DrawOp, x, y, w, h, ppi float64) []point {
	if len(op.Points) > 0 {
...
func fillAttrs(fill *pptxplan.FillStyle) string {
...
func lineAttrs(line *pptxplan.LineStyle) string {
