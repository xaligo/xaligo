...
func TestRenderPlanScalesTextWithRequestedPPI(t *testing.T) {
	plan := pptxplan.Plan{
		Slide: pptxplan.PlanSlide{W: 2, H: 1, Background: "ffffff"},
		Ops: []pptxplan.DrawOp{{
...
func TestRenderPlanRejectsInvalidSlideSize(t *testing.T) {
	_, err := RenderPlan(pptxplan.Plan{}, 96)
	if err == nil {
		t.Fatal("RenderPlan() error = nil, want invalid slide size error")
	}
}
...
func TestRenderPlanUsesCircularRouteMarkers(t *testing.T) {
	plan := pptxplan.Plan{
		...
		Ops: []pptxplan.DrawOp{{
			...
			Line: &pptxplan.LineStyle{Color: "64748B", Width: 1, BeginArrowType: "oval", EndArrowType: "oval"},
		}},
	}
	out, err := RenderPlan(plan, 96)
	if err != nil {
		t.Fatal(err)
	}
	svg := string(out)
