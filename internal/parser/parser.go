...
func Parse(r io.Reader) (model.Document, error) {
	...
	if err != nil {
		return model.Document{}, fmt.Errorf("read DSL: %w", err)
	}
	...
	for {
		...
		if err != nil {
			return model.Document{}, &Error{Position: positionAt(data, offset), Err: fmt.Errorf("parse xml-like token: %w", err)}
		}

		switch t := tok.(type) {
		case xml.StartElement:
			node := &model.Node{Tag: t.Name.Local, Attrs: map[string]string{}, Position: positionAt(data, offset)}
			for _, a := range t.Attr {
				node.Attrs[a.Name.Local] = a.Value
			}
			if node.Tag == "item" {
				if err := validateItemNode(node); err != nil {
					return model.Document{}, &Error{Position: node.Position, Err: fmt.Errorf("parse <item>: %w", err)}
				}
			}
			if node.Tag == "connection" {
				if err := validateConnectionNode(node); err != nil {
					return model.Document{}, &Error{Position: node.Position, Err: fmt.Errorf("parse <connection>: %w", err)}
				}
			}
			if node.Tag == "generic-group" {
				if err := validateGenericGroupNode(node); err != nil {
					return model.Document{}, &Error{Position: node.Position, Err: fmt.Errorf("parse <generic-group>: %w", err)}
				}
			}
			if len(stack) == 0 {
				root = node
		...
		case xml.CharData:
			...
			cur.TextRuns = append(cur.TextRuns, model.TextRun{Text: string(t), Position: positionAt(data, offset)})
			if text != "" {
				if cur.Text == "" {
					cur.Text = text
				} else {
		...
		case xml.EndElement:
			if len(stack) == 0 {
				return model.Document{}, &Error{Position: positionAt(data, offset), Err: fmt.Errorf("unexpected closing tag: %s", t.Name.Local)}
			}
	...
	if root == nil {
		return model.Document{}, &Error{Position: model.Position{Line: 1, Column: 1}, Err: fmt.Errorf("empty document")}
	}
	if root.Tag != "frame" {
		return model.Document{}, &Error{Position: root.Position, Err: fmt.Errorf("root tag must be <frame>, got <%s>", root.Tag)}
	}
...
func validateGenericGroupNode(node *model.Node) error {
...
func expandConnectionShorthands(root *model.Node, data []byte) error {
	aliases := map[string]string{}
	var collect func(*model.Node) error
	collect = func(node *model.Node) error {
		if node.Tag == "item" {
...
func positionAt(data []byte, offset int) model.Position {
	if offset < 0 {
		offset = 0
	}
	if offset > len(data) {
	...
	return model.Position{Offset: offset, Line: line, Column: column}
}
