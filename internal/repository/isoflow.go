...
func (rcvr *isoflowRepository) Render(sceneJSON []byte) ([]byte, error) {
	return rcvr.RenderWithIcons(sceneJSON, nil)
}
...
func embedIconLabel(dataURL, label string) string {
	...
	return share.SVGDataURLFromBytes([]byte(text))
}
...
func (rcvr *tilePlacer) place(preferred entity.IsoflowCoords) entity.IsoflowCoords {
...
func (rcvr *tilePlacer) available(key tileKey) bool {
	for dy := -1; dy <= 1; dy++ {
		for dx := -1; dx <= 1; dx++ {
			if rcvr.occupied[tileKey{x: key.x + dx, y: key.y + dy}] {
				return false
			}
		}
	}
...
func (rcvr *tilePlacer) reserve(key tileKey) {
	rcvr.occupied[key] = true
}
...
func (rcvr *colorRegistry) idFor(color string) string {
	value := isoflowNormalizedColor(color, "#999999")
	if id, ok := rcvr.ids[value]; ok {
		return id
	}
	id := fmt.Sprintf("color%d", len(rcvr.colors)+1)
	rcvr.ids[value] = id
	rcvr.colors = append(rcvr.colors, entity.IsoflowColor{ID: id, Value: value})
	return id
}
