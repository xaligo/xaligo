...
func TestRenderBuildsNestedNodesAndEdges(t *testing.T) {
  ...
  "elements": [
    ...
    {"id":"group-icon","type":"image","x":10,"y":10,"width":32,"height":32,"fileId":"fg"},
  ...
  "appState":{"viewBackgroundColor":"#ffffff"}
	...
	if document.Nodes[0].ID != "group-rect" || document.Nodes[0].Data["icon"] != "data:image/svg+xml;base64,Rw==" {
		t.Fatalf("group icon data = %#v", document.Nodes[0])
	}
	var item Node
	...
	if item.ParentID != "group-rect" || item.Position.X != 40 || item.Position.Y != 70 || item.Data["label"] != "Web" {
