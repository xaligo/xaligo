...
| `component-diagram` | `component`, `interface`, `port`, `artifact` | `dependency`, `realization`, `association`, `assembly`, `delegation` | At least one component. Component boxes use the standard UML component notation with two small rectangles on the left while retaining the xaligo deep-blue/white palette; component child `provided-interface` and `required-interface` entries render as boundary lollipop/socket symbols with outside labels. Required sockets are direct semicircle lines, not masked circles. Realization is component to interface. Assembly uses port/interface endpoints and includes a port. Delegation starts at a port. | [uml-component.xal](../examples/samples/uml-component.xal), [uml-component-connected.xal](../examples/samples/uml-component-connected.xal) |
