# `aws-aurora-oracle-instance-alternate` — Amazon Aurora Oracle Instance Alternate

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Aurora Oracle Instance Alternate](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Database.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1800. Covered catalog IDs: 1800.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-aurora-oracle-instance-alternate
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-aurora-oracle-instance-alternate`。通常の再描画は XAL/README を上書きしない。
