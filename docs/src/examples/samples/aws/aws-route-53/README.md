...
- Default catalog ID: 1233. Covered catalog IDs: 1179, 1197, 1215, 1233.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-route-53
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-route-53`。通常の再描画は XAL/README を上書きしない。
