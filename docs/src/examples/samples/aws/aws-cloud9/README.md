...
![AWS Cloud9](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Developer Tools.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 483. Covered catalog IDs: 435, 451, 467, 483.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-cloud9
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-cloud9`。通常の再描画は XAL/README を上書きしない。
