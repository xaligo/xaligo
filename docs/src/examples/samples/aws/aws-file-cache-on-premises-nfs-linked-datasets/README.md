# `aws-file-cache-on-premises-nfs-linked-datasets` — Amazon File Cache On premises NFS linked datasets

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon File Cache On premises NFS linked datasets](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Storage.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1637. Covered catalog IDs: 1637.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-file-cache-on-premises-nfs-linked-datasets
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `service-context`。サービス文脈: [`aws-file-cache`](../aws-file-cache/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-file-cache-on-premises-nfs-linked-datasets`。通常の再描画は XAL/README を上書きしない。
