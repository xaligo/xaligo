# `aws-aurora-amazon-rds-instance` — Amazon Aurora Amazon RDS Instance

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Aurora Amazon RDS Instance](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Database.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1809. Covered catalog IDs: 1809.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-aurora-amazon-rds-instance
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-aurora-amazon-rds-instance`。通常の再描画は XAL/README を上書きしない。
