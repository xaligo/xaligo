# `aws-sagemaker-ground-truth` — Amazon SageMaker Ground Truth

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon SageMaker Ground Truth](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Artificial Intelligence.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 993. Covered catalog IDs: 870, 911, 952, 993.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-sagemaker-ground-truth
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `service-context`。サービス文脈: [`aws-sagemaker-ground-truth`](../aws-sagemaker-ground-truth/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-sagemaker-ground-truth`。通常の再描画は XAL/README を上書きしない。
