# `aws-opensearch-service-traces` — Amazon OpenSearch Service Traces

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon OpenSearch Service Traces](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Analytics.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1451. Covered catalog IDs: 1451.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-opensearch-service-traces
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-opensearch-service-traces`。通常の再描画は XAL/README を上書きしない。
