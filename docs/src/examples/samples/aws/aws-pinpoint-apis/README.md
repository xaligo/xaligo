# `aws-pinpoint-apis` — Amazon Pinpoint APIs

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Pinpoint APIs](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Business Applications.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 367. Covered catalog IDs: 325, 339, 353, 367.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-pinpoint-apis
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `service-context`。サービス文脈: [`aws-pinpoint-apis`](../aws-pinpoint-apis/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-pinpoint-apis`。通常の再描画は XAL/README を上書きしない。
