...
![AWS IoT Shadow](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1387. Covered catalog IDs: 1387.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-shadow
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-shadow`。通常の再描画は XAL/README を上書きしない。
