# `aws-eventbridge-default-event-bus` — Amazon EventBridge Default Event Bus

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon EventBridge Default Event Bus](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Application Integration.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1496. Covered catalog IDs: 1496.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-eventbridge-default-event-bus
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-eventbridge-default-event-bus`。通常の再描画は XAL/README を上書きしない。
