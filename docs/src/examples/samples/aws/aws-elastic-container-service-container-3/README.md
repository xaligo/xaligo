# `aws-elastic-container-service-container-3` — Amazon Elastic Container Service Container 3

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Elastic Container Service Container 3](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Containers.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1681. Covered catalog IDs: 1681.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-elastic-container-service-container-3
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-elastic-container-service-container-3`。通常の再描画は XAL/README を上書きしない。
