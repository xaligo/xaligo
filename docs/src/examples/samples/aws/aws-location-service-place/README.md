# `aws-location-service-place` — Amazon Location Service Place

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Location Service Place](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Front End Web Mobile.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1855. Covered catalog IDs: 1855.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-location-service-place
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-location-service-place`。通常の再描画は XAL/README を上書きしない。
