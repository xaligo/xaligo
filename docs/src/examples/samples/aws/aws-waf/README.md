...
- Default catalog ID: 308. Covered catalog IDs: 227, 254, 281, 308.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-waf
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-waf`。通常の再描画は XAL/README を上書きしない。
