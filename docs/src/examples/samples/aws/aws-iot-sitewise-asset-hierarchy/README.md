# `aws-iot-sitewise-asset-hierarchy` — AWS IoT SiteWise Asset Hierarchy

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS IoT SiteWise Asset Hierarchy](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1360. Covered catalog IDs: 1360.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-sitewise-asset-hierarchy
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-sitewise-asset-hierarchy`。通常の再描画は XAL/README を上書きしない。
