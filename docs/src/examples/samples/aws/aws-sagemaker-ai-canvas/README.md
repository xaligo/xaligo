# `aws-sagemaker-ai-canvas` — Amazon SageMaker AI Canvas

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon SageMaker AI Canvas](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Artificial Intelligence.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1670. Covered catalog IDs: 1670.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-sagemaker-ai-canvas
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-sagemaker-ai-canvas`。通常の再描画は XAL/README を上書きしない。
