# `aws-fsx-for-netapp-ontap` — Amazon FSx for NetApp ONTAP

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon FSx for NetApp ONTAP](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Storage.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 1061. Covered catalog IDs: 1013, 1029, 1045, 1061.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-fsx-for-netapp-ontap
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `service-context`。サービス文脈: [`aws-fsx-for-netapp-ontap`](../aws-fsx-for-netapp-ontap/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-fsx-for-netapp-ontap`。通常の再描画は XAL/README を上書きしない。
