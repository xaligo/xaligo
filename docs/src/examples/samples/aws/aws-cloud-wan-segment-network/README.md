# `aws-cloud-wan-segment-network` — AWS Cloud WAN Segment Network

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS Cloud WAN Segment Network](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Networking Content Delivery.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1559. Covered catalog IDs: 1559.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-cloud-wan-segment-network
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-cloud-wan-segment-network`。通常の再描画は XAL/README を上書きしない。
