# `aws-iot-greengrass-artifact` — AWS IoT Greengrass Artifact

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS IoT Greengrass Artifact](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1347. Covered catalog IDs: 1347.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-greengrass-artifact
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-greengrass-artifact`。通常の再描画は XAL/README を上書きしない。
