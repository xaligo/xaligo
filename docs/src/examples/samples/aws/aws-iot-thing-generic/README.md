# `aws-iot-thing-generic` — AWS IoT Thing Generic

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS IoT Thing Generic](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1398. Covered catalog IDs: 1398.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-thing-generic
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-thing-generic`。通常の再描画は XAL/README を上書きしない。
