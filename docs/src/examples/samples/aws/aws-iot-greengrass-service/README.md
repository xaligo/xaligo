# `aws-iot-greengrass-service` — AWS IoT Greengrass
...
- Default catalog ID: 429. Covered catalog IDs: 393, 405, 417, 429.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-greengrass-service
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-greengrass-service`。通常の再描画は XAL/README を上書きしない。
