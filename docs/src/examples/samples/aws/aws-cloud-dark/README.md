# `aws-cloud-dark` — AWS Cloud (dark icon)
...
- Default catalog ID: 1863. Covered catalog IDs: 1863.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-cloud-dark
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-cloud-dark`。通常の再描画は XAL/README を上書きしない。
