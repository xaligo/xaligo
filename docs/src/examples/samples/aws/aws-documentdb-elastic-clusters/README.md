# `aws-documentdb-elastic-clusters` — Amazon DocumentDB Elastic Clusters

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon DocumentDB Elastic Clusters](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Database.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1811. Covered catalog IDs: 1811.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-documentdb-elastic-clusters
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-documentdb-elastic-clusters`。通常の再描画は XAL/README を上書きしない。
