# `aws-identity-access-management-iam-roles-anywhere` — AWS Identity Access Management IAM Roles Anywhere

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS Identity Access Management IAM Roles Anywhere](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Security Identity Compliance.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1475. Covered catalog IDs: 1475.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-identity-access-management-iam-roles-anywhere
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-identity-access-management-iam-roles-anywhere`。通常の再描画は XAL/README を上書きしない。
