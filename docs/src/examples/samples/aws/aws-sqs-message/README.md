# `aws-sqs-message` — Amazon Simple Queue Service Message

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Simple Queue Service Message](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Application Integration.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1507. Covered catalog IDs: 1507.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-sqs-message
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-sqs-message`。通常の再描画は XAL/README を上書きしない。
