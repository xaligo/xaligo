...
- Default catalog ID: 1875. Covered catalog IDs: 1875.
...
|---|---|---|---|
| `cidr` | cidr | IPv4/IPv6 CIDR annotation | `10.0.0.0/16` |
...
```sh
npm run generate:aws-samples -- --render --tag=vpc
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=vpc`。通常の再描画は XAL/README を上書きしない。
