# `aws-eventbridge-event` — Amazon EventBridge Event

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon EventBridge Event](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Application Integration.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1494. Covered catalog IDs: 1494.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-eventbridge-event
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-eventbridge-event`。通常の再描画は XAL/README を上書きしない。
