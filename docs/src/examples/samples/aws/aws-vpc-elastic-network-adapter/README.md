# `aws-vpc-elastic-network-adapter` — Amazon VPC Elastic Network Adapter

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon VPC Elastic Network Adapter](sample.svg)
...
- Default catalog ID: 1577. Covered catalog IDs: 1577.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-vpc-elastic-network-adapter
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `service-context`。サービス文脈: [`aws-virtual-private-cloud`](../aws-virtual-private-cloud/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-vpc-elastic-network-adapter`。通常の再描画は XAL/README を上書きしない。
