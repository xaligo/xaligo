...
```sh
npm run generate:aws-samples -- --render --tag=aws-datasync-agent
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `resource-schema`。サービス文脈: [`aws-datasync`](../aws-datasync/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-datasync-agent`。通常の再描画は XAL/README を上書きしない。
