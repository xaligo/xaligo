...
![Amazon Simple Notification Service](sample.svg)
...
- Default catalog ID: 735. Covered catalog IDs: 705, 715, 725, 735.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-sns
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-sns`。通常の再描画は XAL/README を上書きしない。
