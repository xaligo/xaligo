# `aws-vpc-vpn-connection` — Amazon VPC VPN Connection

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon VPC VPN Connection](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Networking Content Delivery.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1589. Covered catalog IDs: 1589.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-vpc-vpn-connection
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-vpc-vpn-connection`。通常の再描画は XAL/README を上書きしない。
