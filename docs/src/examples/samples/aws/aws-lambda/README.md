...
```sh
npm run generate:aws-samples -- --render --tag=aws-lambda
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `service-context`。サービス文脈: [`aws-lambda`](../aws-lambda/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-lambda`。通常の再描画は XAL/README を上書きしない。
