# `aws-cloudwatch-event-event-based` — Amazon CloudWatch Event Event Based

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon CloudWatch Event Event Based](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Management Governance.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1542. Covered catalog IDs: 1542.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-cloudwatch-event-event-based
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-cloudwatch-event-event-based`。通常の再描画は XAL/README を上書きしない。
