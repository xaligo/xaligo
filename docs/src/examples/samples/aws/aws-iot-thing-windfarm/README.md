# `aws-iot-thing-windfarm` — AWS IoT Thing Windfarm

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS IoT Thing Windfarm](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1415. Covered catalog IDs: 1415.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-thing-windfarm
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-thing-windfarm`。通常の再描画は XAL/README を上書きしない。
