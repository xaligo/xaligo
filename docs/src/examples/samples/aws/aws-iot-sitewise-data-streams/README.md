# `aws-iot-sitewise-data-streams` — AWS IoT SiteWise Data Streams

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS IoT SiteWise Data Streams](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1364. Covered catalog IDs: 1364.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-sitewise-data-streams
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-sitewise-data-streams`。通常の再描画は XAL/README を上書きしない。
