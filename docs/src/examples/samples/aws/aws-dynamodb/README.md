...
```sh
npm run generate:aws-samples -- --render --tag=aws-dynamodb
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-dynamodb`。通常の再描画は XAL/README を上書きしない。
