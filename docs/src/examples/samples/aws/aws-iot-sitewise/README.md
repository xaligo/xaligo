...
![AWS IoT SiteWise](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Internet of Things.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 430. Covered catalog IDs: 394, 406, 418, 430.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-sitewise
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-sitewise`。通常の再描画は XAL/README を上書きしない。
