...
![Amazon Managed Streaming for Apache Kafka](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Analytics.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 1160. Covered catalog IDs: 1097, 1118, 1139, 1160.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-managed-streaming-for-apache-kafka
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-managed-streaming-for-apache-kafka`。通常の再描画は XAL/README を上書きしない。
