...
![AWS Trusted Advisor](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Management Governance.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 691. Covered catalog IDs: 601, 631, 661, 691.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-trusted-advisor
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-trusted-advisor`。通常の再描画は XAL/README を上書きしない。
