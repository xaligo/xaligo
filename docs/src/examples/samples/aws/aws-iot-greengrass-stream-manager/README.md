# `aws-iot-greengrass-stream-manager` — AWS IoT Greengrass Stream Manager

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS IoT Greengrass Stream Manager](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1357. Covered catalog IDs: 1357.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-greengrass-stream-manager
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-greengrass-stream-manager`。通常の再描画は XAL/README を上書きしない。
