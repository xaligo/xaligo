# `aws-emr-emr-engine` — Amazon EMR EMR Engine

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon EMR EMR Engine](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Analytics.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1442. Covered catalog IDs: 1442.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-emr-emr-engine
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-emr-emr-engine`。通常の再描画は XAL/README を上書きしない。
