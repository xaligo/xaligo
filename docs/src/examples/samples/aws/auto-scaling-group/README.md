...
- Default catalog ID: 1867. Covered catalog IDs: 1867.
...
```sh
npm run generate:aws-samples -- --render --tag=auto-scaling-group
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=auto-scaling-group`。通常の再描画は XAL/README を上書きしない。
