# `aws-dynamodb-global-secondary-index` — Amazon DynamoDB Global secondary index

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon DynamoDB Global secondary index](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Database.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1815. Covered catalog IDs: 1815.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-dynamodb-global-secondary-index
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-dynamodb-global-secondary-index`。通常の再描画は XAL/README を上書きしない。
