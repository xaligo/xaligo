# `aws-amplify-aws-amplify-studio` — AWS Amplify AWS Amplify Studio

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS Amplify AWS Amplify Studio](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Front End Web Mobile.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1852. Covered catalog IDs: 1852.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-amplify-aws-amplify-studio
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-amplify-aws-amplify-studio`。通常の再描画は XAL/README を上書きしない。
