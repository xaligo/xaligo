...
|---|---|---|---|
| `billing-mode` | `on-demand` / `provisioned` | Billing mode annotation | `on-demand` |
| `partition-key` | text | Partition key annotation | `id` |
...
```sh
npm run generate:aws-samples -- --render --tag=aws-dynamodb-table
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-dynamodb-table`。通常の再描画は XAL/README を上書きしない。
