# `aws-datazone-data-projects` — Amazon DataZone Data Projects

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon DataZone Data Projects](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Analytics.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1440. Covered catalog IDs: 1440.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-datazone-data-projects
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-datazone-data-projects`。通常の再描画は XAL/README を上書きしない。
