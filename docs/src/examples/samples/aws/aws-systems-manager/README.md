...
![AWS Systems Manager](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Management Governance.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 689. Covered catalog IDs: 599, 629, 659, 689.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-systems-manager
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-systems-manager`。通常の再描画は XAL/README を上書きしない。
