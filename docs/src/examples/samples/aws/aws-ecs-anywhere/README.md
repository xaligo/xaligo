# `aws-ecs-anywhere` — Amazon ECS Anywhere

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon ECS Anywhere](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Containers.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 569. Covered catalog IDs: 542, 551, 560, 569.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-ecs-anywhere
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `service-context`。サービス文脈: [`aws-ecs-anywhere`](../aws-ecs-anywhere/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-ecs-anywhere`。通常の再描画は XAL/README を上書きしない。
