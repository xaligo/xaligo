# `aws-sagemaker-ai-notebook` — Amazon SageMaker AI Notebook

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon SageMaker AI Notebook](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Artificial Intelligence.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1673. Covered catalog IDs: 1673.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-sagemaker-ai-notebook
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-sagemaker-ai-notebook`。通常の再描画は XAL/README を上書きしない。
