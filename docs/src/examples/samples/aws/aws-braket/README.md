...
![Amazon Braket](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Quantum Technologies.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 384. Covered catalog IDs: 381, 382, 383, 384.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-braket
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-braket`。通常の再描画は XAL/README を上書きしない。
