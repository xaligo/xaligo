# `aws-identity-access-management-encrypted-data` — AWS Identity Access Management Encrypted Data

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS Identity Access Management Encrypted Data](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Security Identity Compliance.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1473. Covered catalog IDs: 1473.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-identity-access-management-encrypted-data
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-identity-access-management-encrypted-data`。通常の再描画は XAL/README を上書きしない。
