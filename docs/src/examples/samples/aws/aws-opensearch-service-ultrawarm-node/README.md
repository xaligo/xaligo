# `aws-opensearch-service-ultrawarm-node` — Amazon OpenSearch Service UltraWarm Node

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon OpenSearch Service UltraWarm Node](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Analytics.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1452. Covered catalog IDs: 1452.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-opensearch-service-ultrawarm-node
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-opensearch-service-ultrawarm-node`。通常の再描画は XAL/README を上書きしない。
