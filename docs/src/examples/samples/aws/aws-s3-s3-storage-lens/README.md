# `aws-s3-s3-storage-lens` — Amazon Simple Storage Service S3 Storage Lens

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Simple Storage Service S3 Storage Lens](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Storage.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1663. Covered catalog IDs: 1663.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-s3-s3-storage-lens
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-s3-s3-storage-lens`。通常の再描画は XAL/README を上書きしない。
