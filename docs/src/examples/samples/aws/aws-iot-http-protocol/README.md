# `aws-iot-http-protocol` — AWS IoT HTTP Protocol

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS IoT HTTP Protocol](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1377. Covered catalog IDs: 1377.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-http-protocol
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-http-protocol`。通常の再描画は XAL/README を上書きしない。
