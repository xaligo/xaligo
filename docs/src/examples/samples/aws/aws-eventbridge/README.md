...
![Amazon EventBridge](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: App Integration.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 732. Covered catalog IDs: 702, 712, 722, 732.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-eventbridge
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-eventbridge`。通常の再描画は XAL/README を上書きしない。
