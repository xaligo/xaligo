...
```sh
npm run generate:aws-samples -- --render --tag=aws-route-53-resolver
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-route-53-resolver`。通常の再描画は XAL/README を上書きしない。
