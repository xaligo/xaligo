# `aws-iot-device-gateway` — AWS IoT Device Gateway

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS IoT Device Gateway](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1372. Covered catalog IDs: 1372.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-device-gateway
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-device-gateway`。通常の再描画は XAL/README を上書きしない。
