...
![AWS Nitro Enclaves](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Compute.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 90. Covered catalog IDs: 15, 40, 65, 90.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-nitro-enclaves
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `service-context`。サービス文脈: [`aws-nitro-enclaves`](../aws-nitro-enclaves/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-nitro-enclaves`。通常の再描画は XAL/README を上書きしない。
