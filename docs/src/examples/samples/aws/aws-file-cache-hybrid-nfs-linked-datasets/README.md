# `aws-file-cache-hybrid-nfs-linked-datasets` — Amazon File Cache Hybrid NFS linked datasets

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon File Cache Hybrid NFS linked datasets](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Storage.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1636. Covered catalog IDs: 1636.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-file-cache-hybrid-nfs-linked-datasets
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-file-cache-hybrid-nfs-linked-datasets`。通常の再描画は XAL/README を上書きしない。
