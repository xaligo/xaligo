# `aws-iot-thing-temperature-humidity-sensor` — AWS IoT Thing Temperature Humidity Sensor

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS IoT Thing Temperature Humidity Sensor](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1408. Covered catalog IDs: 1408.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-thing-temperature-humidity-sensor
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-thing-temperature-humidity-sensor`。通常の再描画は XAL/README を上書きしない。
