...
![AWS Fargate](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Containers.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 568. Covered catalog IDs: 541, 550, 559, 568.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-fargate
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `service-context`。サービス文脈: [`aws-fargate`](../aws-fargate/README.md)。
...
- [公式資料 3](https://github.com/boto/botocore/blob/develop/botocore/data/eks/2017-11-01/service-2.json)
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-fargate`。通常の再描画は XAL/README を上書きしない。
