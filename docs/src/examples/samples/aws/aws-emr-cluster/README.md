...
![Amazon EMR Cluster](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Analytics.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1441. Covered catalog IDs: 1441.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-emr-cluster
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `resource-schema`。サービス文脈: [`aws-emr`](../aws-emr/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-emr-cluster`。通常の再描画は XAL/README を上書きしない。
