# `aws-athena-data-source-connectors` — Amazon Athena Data Source Connectors

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Athena Data Source Connectors](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Analytics.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1436. Covered catalog IDs: 1436.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-athena-data-source-connectors
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-athena-data-source-connectors`。通常の再描画は XAL/README を上書きしない。
