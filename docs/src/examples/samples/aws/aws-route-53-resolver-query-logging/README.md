# `aws-route-53-resolver-query-logging` — Amazon Route 53 Resolver Query Logging

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Route 53 Resolver Query Logging](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Networking Content Delivery.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1571. Covered catalog IDs: 1571.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-route-53-resolver-query-logging
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-route-53-resolver-query-logging`。通常の再描画は XAL/README を上書きしない。
