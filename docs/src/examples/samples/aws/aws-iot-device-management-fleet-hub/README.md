# `aws-iot-device-management-fleet-hub` — AWS IoT Device Management Fleet Hub

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS IoT Device Management Fleet Hub](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1345. Covered catalog IDs: 1345.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-device-management-fleet-hub
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `service-context`。サービス文脈: [`aws-iot-device-management`](../aws-iot-device-management/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-device-management-fleet-hub`。通常の再描画は XAL/README を上書きしない。
