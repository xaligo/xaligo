...
![AWS DeepComposer](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Artificial Intelligence.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 963. Covered catalog IDs: 840, 881, 922, 963.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-deepcomposer
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `product-guide`。サービス文脈: [`aws-deepcomposer`](../aws-deepcomposer/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-deepcomposer`。通常の再描画は XAL/README を上書きしない。
