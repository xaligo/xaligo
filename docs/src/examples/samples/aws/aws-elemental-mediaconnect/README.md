...
![AWS Elemental MediaConnect](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Media Services.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 821. Covered catalog IDs: 755, 777, 799, 821.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-elemental-mediaconnect
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-elemental-mediaconnect`。通常の再描画は XAL/README を上書きしない。
