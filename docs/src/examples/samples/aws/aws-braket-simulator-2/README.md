# `aws-braket-simulator-2` — Amazon Braket Simulator 2

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Braket Simulator 2](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Quantum Technologies.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1424. Covered catalog IDs: 1424.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-braket-simulator-2
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-braket-simulator-2`。通常の再描画は XAL/README を上書きしない。
