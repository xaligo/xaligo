# `aws-braket-tensor-network` — Amazon Braket Tensor Network

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Braket Tensor Network](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Quantum Technologies.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1429. Covered catalog IDs: 1429.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-braket-tensor-network
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-braket-tensor-network`。通常の再描画は XAL/README を上書きしない。
