# `aws-location-service-routes` — Amazon Location Service Routes

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Location Service Routes](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Front End Web Mobile.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1856. Covered catalog IDs: 1856.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-location-service-routes
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-location-service-routes`。通常の再描画は XAL/README を上書きしない。
