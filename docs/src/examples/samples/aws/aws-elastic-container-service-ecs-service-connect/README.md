# `aws-elastic-container-service-ecs-service-connect` — Amazon Elastic Container Service ECS Service Connect

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Elastic Container Service ECS Service Connect](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Containers.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1683. Covered catalog IDs: 1683.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-elastic-container-service-ecs-service-connect
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-elastic-container-service-ecs-service-connect`。通常の再描画は XAL/README を上書きしない。
