# `aws-vpc-reachability-analyzer` — Amazon VPC Reachability Analyzer

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon VPC Reachability Analyzer](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Networking Content Delivery.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1586. Covered catalog IDs: 1586.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-vpc-reachability-analyzer
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-vpc-reachability-analyzer`。通常の再描画は XAL/README を上書きしない。
