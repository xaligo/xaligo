# `aws-elastic-block-store-amazon-data-lifecycle-manager` — Amazon Elastic Block Store Amazon Data Lifecycle Manager

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Elastic Block Store Amazon Data Lifecycle Manager](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Storage.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1624. Covered catalog IDs: 1624.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-elastic-block-store-amazon-data-lifecycle-manager
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `service-context`。サービス文脈: [`aws-elastic-block-store`](../aws-elastic-block-store/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-elastic-block-store-amazon-data-lifecycle-manager`。通常の再描画は XAL/README を上書きしない。
