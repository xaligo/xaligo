# `aws-route-53-resolver-dns-firewall` — Amazon Route 53 Resolver DNS Firewall

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Route 53 Resolver DNS Firewall](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Networking Content Delivery.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1570. Covered catalog IDs: 1570.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-route-53-resolver-dns-firewall
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-route-53-resolver-dns-firewall`。通常の再描画は XAL/README を上書きしない。
