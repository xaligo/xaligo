# `aws-transit-gateway-attachment` — AWS Transit Gateway Attachment

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS Transit Gateway Attachment](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Networking Content Delivery.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1562. Covered catalog IDs: 1562.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-transit-gateway-attachment
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `service-context`。サービス文脈: [`aws-transit-gateway`](../aws-transit-gateway/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-transit-gateway-attachment`。通常の再描画は XAL/README を上書きしない。
