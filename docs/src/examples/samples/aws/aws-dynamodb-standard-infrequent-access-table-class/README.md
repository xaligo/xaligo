# `aws-dynamodb-standard-infrequent-access-table-class` — Amazon DynamoDB Standard Infrequent Access Table Class

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon DynamoDB Standard Infrequent Access Table Class](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Database.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1819. Covered catalog IDs: 1819.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-dynamodb-standard-infrequent-access-table-class
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-dynamodb-standard-infrequent-access-table-class`。通常の再描画は XAL/README を上書きしない。
