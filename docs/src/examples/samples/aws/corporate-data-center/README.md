...
![Corporate data center](sample.svg)
...
- Default catalog ID: 1868. Covered catalog IDs: 1868.
...
```sh
npm run generate:aws-samples -- --render --tag=corporate-data-center
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=corporate-data-center`。通常の再描画は XAL/README を上書きしない。
