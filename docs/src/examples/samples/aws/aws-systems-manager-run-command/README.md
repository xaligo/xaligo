...
![AWS Systems Manager Run Command](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Management Governance.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1531. Covered catalog IDs: 1531.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-systems-manager-run-command
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-systems-manager-run-command`。通常の再描画は XAL/README を上書きしない。
