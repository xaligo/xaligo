...
- Default catalog ID: 1872. Covered catalog IDs: 1872.
...
|---|---|---|---|
| `region-name` | text | AWS Region name | `ap-northeast-1` |
...
```sh
npm run generate:aws-samples -- --render --tag=region
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=region`。通常の再描画は XAL/README を上書きしない。
