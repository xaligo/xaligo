# `aws-elastic-block-store` — Amazon Elastic Block Store

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Elastic Block Store](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Storage.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 1059. Covered catalog IDs: 1011, 1027, 1043, 1059.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-elastic-block-store
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-elastic-block-store`。通常の再描画は XAL/README を上書きしない。
