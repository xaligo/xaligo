...
![Amazon Simple Storage Service](sample.svg)
...
- Default catalog ID: 1068. Covered catalog IDs: 1020, 1036, 1052, 1068.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-s3
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-s3`。通常の再描画は XAL/README を上書きしない。
