...
![AWS Glue Data Catalog](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Analytics.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1433. Covered catalog IDs: 1433.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-glue-data-catalog
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-glue-data-catalog`。通常の再描画は XAL/README を上書きしない。
