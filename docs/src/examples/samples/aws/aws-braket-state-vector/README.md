# `aws-braket-state-vector` — Amazon Braket State Vector

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Braket State Vector](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Quantum Technologies.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1428. Covered catalog IDs: 1428.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-braket-state-vector
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-braket-state-vector`。通常の再描画は XAL/README を上書きしない。
