# `aws-s3-s3-batch-operations` — Amazon Simple Storage Service S3 Batch Operations

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Simple Storage Service S3 Batch Operations](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Storage.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1646. Covered catalog IDs: 1646.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-s3-s3-batch-operations
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-s3-s3-batch-operations`。通常の再描画は XAL/README を上書きしない。
