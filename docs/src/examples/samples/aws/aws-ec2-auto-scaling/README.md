# `aws-ec2-auto-scaling` — Amazon EC2 Auto Scaling
...
- Default catalog ID: 100. Covered catalog IDs: 25, 50, 75, 100.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-ec2-auto-scaling
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-ec2-auto-scaling`。通常の再描画は XAL/README を上書きしない。
