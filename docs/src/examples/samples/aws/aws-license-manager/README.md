...
![AWS License Manager](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Management Governance.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 681. Covered catalog IDs: 591, 621, 651, 681.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-license-manager
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-license-manager`。通常の再描画は XAL/README を上書きしない。
