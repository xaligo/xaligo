# `aws-iot-fire-tv-stick` — AWS IoT Fire TV Stick

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS IoT Fire TV Stick](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1374. Covered catalog IDs: 1374.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-fire-tv-stick
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-fire-tv-stick`。通常の再描画は XAL/README を上書きしない。
