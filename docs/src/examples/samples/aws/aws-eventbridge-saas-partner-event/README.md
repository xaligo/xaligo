# `aws-eventbridge-saas-partner-event` — Amazon EventBridge Saas Partner Event

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon EventBridge Saas Partner Event](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Application Integration.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1499. Covered catalog IDs: 1499.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-eventbridge-saas-partner-event
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-eventbridge-saas-partner-event`。通常の再描画は XAL/README を上書きしない。
