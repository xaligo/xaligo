# `aws-iot-greengrass-interprocess-communication` — AWS IoT Greengrass Interprocess Communication

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS IoT Greengrass Interprocess Communication](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1354. Covered catalog IDs: 1354.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-greengrass-interprocess-communication
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-greengrass-interprocess-communication`。通常の再描画は XAL/README を上書きしない。
