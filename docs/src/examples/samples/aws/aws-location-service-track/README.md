# `aws-location-service-track` — Amazon Location Service Track 

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Location Service Track ](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Front End Web Mobile.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1857. Covered catalog IDs: 1857.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-location-service-track
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-location-service-track`。通常の再描画は XAL/README を上書きしない。
