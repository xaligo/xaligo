# `aws-sns-email-notification` — Amazon Simple Notification Service Email Notification

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Simple Notification Service Email Notification](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Application Integration.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1504. Covered catalog IDs: 1504.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-sns-email-notification
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `service-context`。サービス文脈: [`aws-sns`](../aws-sns/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-sns-email-notification`。通常の再描画は XAL/README を上書きしない。
