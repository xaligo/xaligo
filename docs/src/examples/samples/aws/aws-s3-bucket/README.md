...
![Amazon Simple Storage Service Bucket](sample.svg)

Regional storage service/bucket. Access via a VPC endpoint does not place the bucket inside a subnet.

- Kind: `resource`; category: Storage.
- Diagram scope: `region` (recommendation, not AWS deployment validation).
- Default catalog ID: 1642. Covered catalog IDs: 1642.
...
|---|---|---|---|
| `bucket-name` | text | Bucket name annotation | `example-assets` |
| `versioning` | boolean | Versioning annotation | `true` |
...
```sh
npm run generate:aws-samples -- --render --tag=aws-s3-bucket
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `resource-schema`。サービス文脈: [`aws-s3`](../aws-s3/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-s3-bucket`。通常の再描画は XAL/README を上書きしない。
