...
- Default catalog ID: 1862. Covered catalog IDs: 1862.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-cloud
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-cloud`。通常の再描画は XAL/README を上書きしない。
