# `aws-aurora-piops-instance` — Amazon Aurora PIOPS Instance

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Aurora PIOPS Instance](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Database.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1802. Covered catalog IDs: 1802.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-aurora-piops-instance
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-aurora-piops-instance`。通常の再描画は XAL/README を上書きしない。
