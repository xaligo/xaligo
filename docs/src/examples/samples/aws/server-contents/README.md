...
- Default catalog ID: 1873. Covered catalog IDs: 1873.
...
```sh
npm run generate:aws-samples -- --render --tag=server-contents
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=server-contents`。通常の再描画は XAL/README を上書きしない。
