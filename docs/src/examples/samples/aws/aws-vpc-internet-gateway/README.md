# `aws-vpc-internet-gateway` — Amazon VPC Internet Gateway

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon VPC Internet Gateway](sample.svg)
...
- Default catalog ID: 1581. Covered catalog IDs: 1581.
- Implementation: V1 and V2; icon-only boundary port, excluded from normal layout flow.

## Parameters

`id` is required. `side` = top/right/bottom/left (default top); `anchor` = 0..1; `offset` = finite tangent displacement; `size` > 0 (default 48). The tag must be an empty direct child of `<vpc>`. Functional annotations are preserved in source/project metadata; the boundary icon has no text label.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-vpc-internet-gateway
```
...
### 構成モデル: `AWS::EC2::InternetGateway`

[公式リファレンス](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-ec2-internetgateway.html)。全 1 プロパティを型付きで列挙します（表示カードには主要項目のみ）。

| Field | Type | Required in AWS schema |
|---|---|---|
| [Tags](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-ec2-internetgateway.html#cfn-ec2-internetgateway-tags) | `List<Tag>` | no |
...
- [公式資料 1](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-ec2-internetgateway.html)
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-vpc-internet-gateway`。通常の再描画は XAL/README を上書きしない。
