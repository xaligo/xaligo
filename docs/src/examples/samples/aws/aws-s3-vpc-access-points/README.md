# `aws-s3-vpc-access-points` — Amazon Simple Storage Service VPC Access Points

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Simple Storage Service VPC Access Points](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Storage.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1666. Covered catalog IDs: 1666.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-s3-vpc-access-points
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-s3-vpc-access-points`。通常の再描画は XAL/README を上書きしない。
