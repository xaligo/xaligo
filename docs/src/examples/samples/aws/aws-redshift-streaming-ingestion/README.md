# `aws-redshift-streaming-ingestion` — Amazon Redshift Streaming Ingestion

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Redshift Streaming Ingestion](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Analytics.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1461. Covered catalog IDs: 1461.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-redshift-streaming-ingestion
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-redshift-streaming-ingestion`。通常の再描画は XAL/README を上書きしない。
