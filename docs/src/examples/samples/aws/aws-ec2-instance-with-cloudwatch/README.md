# `aws-ec2-instance-with-cloudwatch` — Amazon EC2 Instance with CloudWatch

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon EC2 Instance with CloudWatch](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Compute.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1789. Covered catalog IDs: 1789.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-ec2-instance-with-cloudwatch
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-ec2-instance-with-cloudwatch`。通常の再描画は XAL/README を上書きしない。
