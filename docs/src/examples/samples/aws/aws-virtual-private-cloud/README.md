...
![Amazon Virtual Private Cloud](sample.svg)

AWS service icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `service`; category: Networking Content Delivery.
- Diagram scope: `service` (recommendation, not AWS deployment validation).
- Default catalog ID: 1235. Covered catalog IDs: 1181, 1199, 1217, 1235.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-virtual-private-cloud
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-virtual-private-cloud`。通常の再描画は XAL/README を上書きしない。
