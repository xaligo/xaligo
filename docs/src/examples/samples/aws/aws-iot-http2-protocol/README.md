# `aws-iot-http2-protocol` — AWS IoT HTTP2 Protocol

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS IoT HTTP2 Protocol](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1376. Covered catalog IDs: 1376.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-http2-protocol
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-http2-protocol`。通常の再描画は XAL/README を上書きしない。
