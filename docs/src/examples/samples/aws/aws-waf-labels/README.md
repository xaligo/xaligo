# `aws-waf-labels` — AWS WAF Labels

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS WAF Labels](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Security Identity Compliance.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1489. Covered catalog IDs: 1489.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-waf-labels
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-waf-labels`。通常の再描画は XAL/README を上書きしない。
