# `aws-elasticache-cache-node` — Amazon ElastiCache Cache Node

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon ElastiCache Cache Node](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Database.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1822. Covered catalog IDs: 1822.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-elasticache-cache-node
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-elasticache-cache-node`。通常の再描画は XAL/README を上書きしない。
