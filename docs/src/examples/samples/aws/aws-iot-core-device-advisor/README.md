# `aws-iot-core-device-advisor` — AWS IoT Core Device Advisor

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![AWS IoT Core Device Advisor](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: IoT.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1342. Covered catalog IDs: 1342.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-iot-core-device-advisor
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-iot-core-device-advisor`。通常の再描画は XAL/README を上書きしない。
