# `aws-route-53-hosted-zone` — Amazon Route 53 Hosted Zone

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon Route 53 Hosted Zone](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Networking Content Delivery.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1568. Covered catalog IDs: 1568.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-route-53-hosted-zone
```

<!-- aws-functional-research:start -->
## 機能調査・構成デザイン（2026-09-06）

分類: `resource-schema`。サービス文脈: [`aws-route-53`](../aws-route-53/README.md)。
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-route-53-hosted-zone`。通常の再描画は XAL/README を上書きしない。
