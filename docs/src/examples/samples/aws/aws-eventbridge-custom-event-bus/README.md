# `aws-eventbridge-custom-event-bus` — Amazon EventBridge Custom Event Bus

[SVG preview](sample.svg) · [Editable XAL](sample.xal) · [Catalog](../README.md)

![Amazon EventBridge Custom Event Bus](sample.svg)

AWS resource icon. Use a label and explicit annotations to describe its role; scope is selected by the author.

- Kind: `resource`; category: Application Integration.
- Diagram scope: `logical` (recommendation, not AWS deployment validation).
- Default catalog ID: 1495. Covered catalog IDs: 1495.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-eventbridge-custom-event-bus
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-eventbridge-custom-event-bus`。通常の再描画は XAL/README を上書きしない。
