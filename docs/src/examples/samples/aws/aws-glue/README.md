...
- Default catalog ID: 1148. Covered catalog IDs: 1085, 1106, 1127, 1148.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-glue
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-glue`。通常の再描画は XAL/README を上書きしない。
