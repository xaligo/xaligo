...
```sh
npm run generate:aws-samples -- --render --tag=aws-waf-bot
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-waf-bot`。通常の再描画は XAL/README を上書きしない。
