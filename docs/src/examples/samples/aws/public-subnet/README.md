...
- Default catalog ID: 1871. Covered catalog IDs: 1871.
...
```sh
npm run generate:aws-samples -- --render --tag=public-subnet
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=public-subnet`。通常の再描画は XAL/README を上書きしない。
