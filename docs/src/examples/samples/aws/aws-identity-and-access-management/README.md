...
![AWS Identity and Access Management](sample.svg)
...
- Default catalog ID: 297. Covered catalog IDs: 216, 243, 270, 297.
...
```sh
npm run generate:aws-samples -- --render --tag=aws-identity-and-access-management
```
...
- 編集後は `npm run generate:aws-samples -- --render --tag=aws-identity-and-access-management`。通常の再描画は XAL/README を上書きしない。
